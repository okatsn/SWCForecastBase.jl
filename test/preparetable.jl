...
@testset "preparetable.jl: interface" begin
    using SWCExampleDatasets
    CA = ConfigAccumulate()
    @test isa(CA.variables, Cols)
    @test isempty(CA.unit)
    @test isa(CA.intervals, Vector{<:Int})

    CA1 = ConfigAccumulate(; unit="day", intervals=[99])
    @test !isempty(CA1.unit)
    @test length(CA1.intervals) == 1
    @test_throws MethodError ConfigAccumulate(; intervals=[99.0])
end
